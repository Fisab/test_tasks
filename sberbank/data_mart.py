...
class DataMart:
	...
	def _retrieve_data(self, url_key):
		data = None
	...
	def retrieve_regions(self):
		raw_regions = self._retrieve_data('regions')
		self.regions = pd.read_excel(BytesIO(raw_regions))
		self.regions['District'] = self.regions.District.apply(lambda x: x.strip())
		return self.regions

	def _prepare_child_clinic(self):
		clinic_ch = self._retrieve_data('childens_clinic')
	...
	def _prepare_education(self):
		educ = self._retrieve_data('education')
	...
	def _prepare_social_food(self):
		food = self._retrieve_data('social_food')
		df_food = pd.DataFrame(food)
		self.df_food_count = df_food[['Name', 'District']].groupby('District').count() \
			.reset_index() \
			.rename({'Name': 'FoodCount'}, axis=1)

	def _prepare_rescue(self):
		rescue = self._retrieve_data('rescue_garrison')
	...
	def _prepare_swimming_pools(self):
		swim_p = self._retrieve_data('swimming_pool')
		self.df_swim_p = pd.DataFrame(swim_p)

	def _prepare_emergency(self):
		emergency = self._retrieve_data('emergency_services')
	...
	def prepare_data_mart(self):
		"""
		Run all function to retrieve data and then run func to merge it
		:return:
		"""
		# load and prepare all data
		self._prepare_child_clinic()
		self._prepare_education()
		self._prepare_social_food()
		self._prepare_rescue()
		self._prepare_swimming_pools()
		self._prepare_emergency()
